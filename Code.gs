 ...
 * DISC_MAP corregido · Hojas completas según estructura original
 */
...
// ── DISC_MAP CORREGIDO (Factor_BUR original) ──────────────────────────────────
const DISC_MAP = {
  // D — Dominancia
  ...
  'PERSISTENTE':'D','CONFIADO':'D',
  'AFIRMATIVO':'D',    // corregido (era C)
  'CONSECUENTE':'D',   // corregido (era C)
  'NERVIOSO':'D',      // corregido (era I)
  // I — Influencia
  ...
  'INSPIRADOR':'I','SIMPATIZADOR':'I',
  'ANIMOSO':'I',       // corregido (era D)
  'DESEOSO':'I',       // corregido (era D)
  'REFINADO':'I',      // corregido (era C)
  // S — Estabilidad
  ...
  'BONDADOSO':'S','COMPLACIENTE':'S',
  'GENEROSO':'S',      // corregido (era I)
  'TIMIDO':'S',        // corregido (era C)
  'INDIFERENTE':'S',   // corregido (era C)
  'MODERADO':'S',      // corregido (era C)
  // C — Cumplimiento
  ...
  'HUMILDE':'C',       // corregido (era S)
  'DEVOTO':'C',        // corregido (era S)
  'OBEDIENTE':'C',     // corregido (era S)
  'RESPETUOSO':'C',    // corregido (era S)
  'CONVENCIONAL':'C'   // corregido (era S)
};

// ── DATOS POR FACTOR (texto para gráficas M/L/T) ─────────────────────────────
const DISC_DATOS = {
  D: {
    ...
    textoT: 'El perfil total muestra un estilo conductual dominante, de alta energía y orientación al logro. Prefiere ambientes de trabajo retadores con autonomía.',
    ...
    recomendaciones: 'Asignar proyectos con alta responsabilidad y autonomía. Proporcionar metas claras y métricas de éxito. Evitar microgestión.',
    alertas: 'Puede ignorar el proceso y saltarse pasos. Tiende a imponer decisiones sin consenso. Riesgo de conflicto con pares de igual perfil.'
  },
  I: {
    ...
    textoT: 'El perfil total refleja un estilo comunicativo, relacional y orientado a las personas. Brilla en entornos colaborativos y de alta visibilidad.',
    ...
    motivadores: 'Reconocimiento público, libertad de expresión, trabajo en equipo dinámico, variedad y novedad en tareas.',
  ...
  S: {
    ...
    textoL: 'Su imagen adaptativa proyecta calma, disponibilidad y apoyo. Puede ser percibida como pasiva o sin iniciativa si no se le da espacio para contribuir.',
    textoT: 'El perfil total muestra un estilo colaborativo, estable y orientado a las relaciones de largo plazo. Prefiere ambientes predecibles con rutinas establecidas.',
    ...
    fortalezas: ['Lealtad y confiabilidad sostenida en el tiempo','Excelente trabajo en equipo y colaboración','Paciencia y consistencia en la ejecución','Estabilidad emocional bajo presión','Apoyo genuino sin buscar reconocimiento'],
    areas: ['Resistencia al cambio rápido o no planificado','Dificultad para establecer límites y decir no','Tendencia a postergar conversaciones difíciles','Exceso de complacencia que limita crecimiento'],
    motivadores: 'Estabilidad laboral, relaciones de confianza, reconocimiento de lealtad, entornos armoniosos y predecibles.',
    limitaciones: 'Resistencia al cambio, dificultad para confrontar, puede ser aprovechado por otros, lentitud para adaptarse.',
    recomendaciones: 'Dar tiempo de adaptación ante cambios. Reconocer explícitamente su contribución. Fomentar que exprese opiniones. Evitar sobrecarga silenciosa.',
  ...
  C: {
    textoM: 'Se autopercibe como persona precisa, analítica y orientada a la calidad. En su mejor versión sus entregas son impecables y sus análisis son profundos y confiables.',
    textoL: 'Su imagen adaptativa proyecta rigor, metodología y alto estándar. Puede percibirse como rígido, crítico o difícil de complacer en entornos más informales.',
    textoT: 'El perfil total refleja un estilo analítico, detallista y orientado al cumplimiento. Prefiere entornos donde los procesos están bien definidos y se respetan.',
    ...
    areas: ['Parálisis por análisis excesivo antes de actuar','Perfeccionismo que puede frenar la ejecución','Rigidez ante procedimientos que cambian','Comunicación percibida como fría o distante'],
    motivadores: 'Procesos claros, información completa, estándares de calidad, reconocimiento por precisión y corrección.',
    limitaciones: 'Lentitud por exceso de análisis, rigidez, dificultad para improvisar, puede ser percibido como crítico en exceso.',
...
// ── PERFILES DISC (16 combinaciones completas) ────────────────────────────────
const PERFILES_DISC = {
  'D': {
    nombre:'Director', segmento:'Dirección General / Alta Dirección / Emprendimiento',
    resumen:'Perfil de alto mando orientado a resultados. Toma decisiones firmes, opera a ritmo acelerado y lidera con autoridad. Le motiva ganar y superar metas constantemente.',
    ...
    caract:'Directo, competitivo, independiente, orientado a resultados, alto ritmo, tolerante al riesgo, baja tolerancia a la burocracia.',
  ...
  'DI': {
    nombre:'Promotor', segmento:'Ventas / Desarrollo de Negocios / Marketing',
    resumen:'Combina liderazgo directivo con carisma social. Enérgico, persuasivo y orientado a resultados. Mueve equipos con entusiasmo y visión.',
    desc:'Enérgico, persuasivo y orientado a resultados. Combina liderazgo con carisma para motivar y mover equipos hacia metas ambiciosas con entusiasmo.',
    caract:'Enérgico, carismático, orientado a resultados, persuasivo, emprendedor, rápido en decisiones, fuerte presencia.',
  ...
  'DS': {
    nombre:'Desarrollador', segmento:'Gestión de Proyectos / Producción / Operaciones',
    resumen:'Decidido y orientado a las personas. Establece metas firmes y construye relaciones de largo plazo para alcanzarlas de forma sostenida.',
    desc:'Decidido y orientado a las personas. Establece metas firmes y construye relaciones de largo plazo para alcanzarlas de forma sostenida y consistente.',
  ...
  'DC': {
    nombre:'Perfeccionista', segmento:'Finanzas / Auditoría / Dirección de Calidad',
    resumen:'Exigente consigo mismo y con los demás. Busca excelencia, control y precisión. Analiza antes de actuar pero actúa con determinación.',
    desc:'Exigente consigo mismo y con los demás. Busca excelencia, control y precisión sobre los resultados. Analiza antes de actuar pero actúa con determinación.',
    ...
    limitaciones:'Puede ser percibido como inflexible. Dificultad para trabajar con perfiles más relajados. Lento para decidir en incertidumbre.',
    recomendaciones:'Roles de control y dirección con impacto en calidad. Dar datos completos. Respetar su ritmo analítico.',
    alertas:'Puede bloquear equipos por estándares muy altos. Conflicto con perfiles I o S que no comparten su rigor.'
  },
  'I': {
    nombre:'Inspirador', segmento:'Comunicación / Relaciones Públicas / Formación',
    resumen:'Comunicador nato con energía positiva contagiosa. Motiva a otros, disfruta entornos sociales y genera entusiasmo y creatividad en el equipo.',
    desc:'Comunicador nato con energía positiva contagiosa. Motiva a otros, disfruta los entornos sociales y genera entusiasmo y creatividad en el equipo.',
    caract:'Sociable, optimista, carismático, creativo, motivador, orientado a las personas, alto impacto en cultura.',
    motivadores:'Reconocimiento, libertad de expresión, interacción social, variedad, creatividad.',
  ...
  'ID': {
    nombre:'Agente de Cambio', segmento:'Liderazgo Comercial / Emprendimiento / Innovación',
    resumen:'Dinámico e influyente. Combina energía social con capacidad de acción directa. Impulsa iniciativas con visión y entusiasmo.',
    desc:'Dinámico e influyente. Combina energía social con capacidad de acción directa. Impulsa iniciativas y arrastra al equipo con visión y entusiasmo.',
    ...
    alertas:'Alto riesgo de agotamiento si hay demasiados frentes abiertos. Necesita foco.'
  },
  'IS': {
    nombre:'Consejero', segmento:'Recursos Humanos / Capacitación / Atención al Cliente',
    resumen:'Cálido, empático y profundamente orientado a las personas. Excelente construyendo relaciones de confianza y ambientes colaborativos.',
    desc:'Cálido, empático y profundamente orientado a las personas. Excelente construyendo relaciones de confianza y generando ambientes de trabajo colaborativos y positivos.',
    caract:'Empático, sociable, cálido, orientado al bienestar del equipo, comunicador efectivo, construye confianza.',
    motivadores:'Relaciones genuinas, impacto en personas, reconocimiento de su contribución al equipo, armonía.',
    limitaciones:'Evita conflictos, puede sacrificar metas por relaciones, difícil con personas difíciles.',
    recomendaciones:'Roles de consultoría interna, RRHH, atención al cliente o capacitación. Fomentar asertividad.',
    alertas:'Puede retener información negativa para no herir. Necesita desarrollo en habilidades de confrontación sana.'
  },
  'IC': {
    nombre:'Evaluador', segmento:'Marketing / Consultoría Comunicacional / Análisis de Audiencias',
    resumen:'Analítico y sociable. Combina precisión y rigor con habilidad para comunicar ideas complejas de forma accesible.',
    desc:'Analítico y sociable. Combina precisión y rigor con habilidad para comunicar ideas complejas de forma accesible y convincente para su audiencia.',
  ...
  'S': {
    nombre:'Especialista', segmento:'Administración / Soporte Operativo / Logística',
    resumen:'Estable, leal y confiable. Brinda soporte constante al equipo, trabaja mejor en entornos con relaciones sólidas y ritmo predecible.',
    desc:'Estable, leal y confiable. Brinda soporte constante al equipo, trabaja mejor en entornos con relaciones sólidas, expectativas claras y ritmo predecible.',
    caract:'Leal, paciente, consistente, orientado al servicio, bajo perfil, soporte del equipo, alta estabilidad.',
  ...
  'SD': {
    nombre:'Investigador', segmento:'Supervisión / Coordinación Operativa / Proyectos',
    resumen:'Metódico y orientado a las personas. Analiza situaciones antes de actuar, pero cuando lo hace es firme y consistente.',
    desc:'Metódico y orientado a las personas. Analiza situaciones antes de actuar, pero cuando lo hace es firme, consistente y genera confianza en su entorno.',
  ...
  'SI': {
    nombre:'Armonizador', segmento:'Trabajo Social / Psicología Organizacional / RRHH',
    resumen:'Cooperativo y comunicativo. Facilita el trabajo en equipo, construye acuerdos naturalmente y prefiere el consenso sobre la confrontación.',
    desc:'Cooperativo y comunicativo. Facilita el trabajo en equipo, construye acuerdos naturalmente y prefiere el consenso y la armonía sobre la confrontación.',
  ...
  'SC': {
    nombre:'Coordinador', segmento:'Control de Calidad / Procesos / Administración',
    resumen:'Sistemático y estable. Excelente planificando y ejecutando procesos con consistencia, rigor y alto nivel de cumplimiento.',
    desc:'Sistemático y estable. Excelente planificando y ejecutando procesos con consistencia, rigor y alto nivel de cumplimiento. Confiable en cada entrega.',
    caract:'Metódico, confiable, orientado a procesos, alta calidad en entrega, bajo perfil, alta consistencia.',
  ...
  'C': {
    nombre:'Analista', segmento:'Auditoría / Finanzas / Sistemas / Control de Calidad',
    resumen:'Detallista, preciso y orientado a la calidad. Evalúa cuidadosamente antes de decidir y prefiere datos y hechos sobre la intuición.',
    desc:'Detallista, preciso y orientado a la calidad. Evalúa cuidadosamente antes de tomar decisiones y prefiere datos y hechos comprobados sobre la intuición.',
    caract:'Analítico, meticuloso, orientado a datos, alto estándar, sistemático, introvertido, pensamiento crítico.',
  ...
  'CD': {
    nombre:'Observador', segmento:'Control de Gestión / Dirección Analítica / Consultoría Estratégica',
    resumen:'Cauteloso y orientado a resultados. Evalúa riesgos antes de decidir y asegura que cada acción esté respaldada por análisis sólido.',
    desc:'Cauteloso y orientado a resultados. Evalúa riesgos antes de decidir y asegura que cada acción esté respaldada por análisis sólido y evidencia objetiva.',
    ...
    limitaciones:'Puede ser lento si falta información, puede parecer frío o distante, difícil de complacer.',
    recomendaciones:'Roles de control estratégico o consultoría. Dar acceso a datos. Respetar su ritmo.',
    alertas:'Puede generar fricción si cuestiona decisiones sin datos. Necesita respaldo informacional sólido.'
  },
  'CI': {
    nombre:'Diplomático', segmento:'Consultoría / Asesoría Técnica / Docencia / Comunicación Técnica',
    resumen:'Preciso y sociable. Combina rigor analítico con habilidades de comunicación. Excelente mediador en situaciones complejas.',
    desc:'Preciso y sociable. Combina rigor analítico con habilidades de comunicación. Excelente mediador en situaciones complejas, negociador efectivo y empático.',
    ...
    limitaciones:'Puede sobre-analizar respuestas en conversaciones, lento para cerrar acuerdos si falta información.',
  ...
  'CS': {
    nombre:'Defensor', segmento:'Cumplimiento Normativo / Gestión de Riesgos / Calidad y Bienestar',
    resumen:'Meticuloso y empático. Asegura que los procesos sean correctos y que las personas estén bien atendidas. Une el rigor con el cuidado genuino.',
    desc:'Meticuloso y empático. Asegura que los procesos sean correctos y que las personas estén bien atendidas. Une el rigor con el cuidado genuino por otros.',
    caract:'Meticuloso, empático, orientado a calidad y bienestar, confiable, coherente, bajo perfil.',
...
function guardarRespuestas(payload) {
  try {
    ...
    // ── Construir fila de respuestas crudas ──────────────────────────────────
    var filaBase = [];
    payload.respuestas.forEach(function(r) { filaBase.push(r.mas); filaBase.push(r.menos); });

    // ── Calcular perfil DISC ─────────────────────────────────────────────────
    const disc = calcularPerfilDISC(payload.respuestas);

    // ── Validar estado ────────────────────────────────────────────────────────
    var estado = 'OK'; var detalle = 'Evaluación válida y completa.';
    var colisiones = payload.respuestas.filter(function(r){ return r.mas === r.menos; });
    if (colisiones.length > 0) { estado = 'INVÁLIDA'; detalle = 'MÁS = MENOS en bloque(s): ' + colisiones.map(function(r){ return r.bloque; }).join(', '); }
    else if (_yaRespondio(payload.correo)) { estado = 'DUPLICADA'; detalle = 'El correo ya tenía una respuesta registrada.'; }

    // ── Guardar en hoja correcta por tipo ────────────────────────────────────
    if (esCandidato) {
      const h = _getOrCreateSheet(ss, CFG_APP.HOJA_CANDIDATOS, HEADS_CANDIDATOS);
      var tokenId = payload.tokenFila ? 'CAND-' + String(payload.tokenFila).padStart(4,'0') : '';
      h.appendRow([fecha, tokenId, payload.nombre, payload.correo, '', payload.puesto].concat(filaBase));
    } else {
      const h = _getOrCreateSheet(ss, CFG_APP.HOJA_PLANTILLA, HEADS_PLANTILLA);
      h.appendRow([fecha, payload.correo, payload.nombre, payload.puesto].concat(filaBase));
    }

    // ── CONCENTRADO ──────────────────────────────────────────────────────────
    const hConc = _getOrCreateSheet(ss, CFG_APP.HOJA_CONCENTRADO, HEADS_CONCENTRADO);
    hConc.appendRow([tipo, fecha, payload.correo, payload.nombre, payload.puesto].concat(filaBase));

    // ── VALIDACION ────────────────────────────────────────────────────────────
    const hVal = _getOrCreateSheet(ss, CFG_APP.HOJA_VALIDACION, HEADS_VALIDACION);
    var valFila = hVal.appendRow([fecha, payload.correo, payload.nombre, tipo, estado, detalle]);
    if (estado === 'INVÁLIDA')  hVal.getRange(hVal.getLastRow(), 5).setBackground('#FECACA');
    else if (estado === 'DUPLICADA') hVal.getRange(hVal.getLastRow(), 5).setBackground('#FEF08A');
    else                         hVal.getRange(hVal.getLastRow(), 5).setBackground('#BBF7D0');

    // ── CALCULOS ──────────────────────────────────────────────────────────────
    const idGen = esCandidato ? ('CAND-' + String(payload.tokenFila || 0).padStart(4,'0')) : payload.correo;
    ...
    const c1    = disc.combs[0] || {};
    const c2    = disc.combs[1] || {};
    const c3    = disc.combs[2] || {};
    hInt.appendRow([
      ...
      disc.perfil.caract, disc.perfil.motivadores, disc.perfil.limitaciones, disc.perfil.recomendaciones, disc.perfil.alertas,
    ...
    Logger.log("DISC guardado: " + payload.nombre + " — " + disc.perfil.nombre + " (" + tipo + ")");
...
// ── HELPER: obtener o crear hoja con encabezados ──────────────────────────────
...
function calcularPerfilDISC(respuestas) {
  ...
  const perfil = PERFILES_DISC[key] || PERFILES_DISC[top] ||
    {nombre:'Equilibrado', segmento:'Multifuncional', resumen:'Perfil balanceado con alta adaptabilidad.',
     desc:'Perfil balanceado con flexibilidad para adaptarse a distintos contextos.',
     caract:'Adaptable, flexible, multifuncional.', motivadores:'Variedad y equilibrio.',
     limitaciones:'Puede carecer de estilo propio definido.', recomendaciones:'Roles versátiles con variedad de tareas.',
     alertas:'Puede ser percibido como inconsistente.'};

  // Datos de factor dominante para textos
  const topD = DISC_DATOS[top] || {};
  const secD = DISC_DATOS[sec] || {};

  // Fortalezas y áreas mezcladas
  ...
  // Textos de gráfica
  const textoM    = topD.textoM    || '';
  const textoL    = topD.textoL    || '';
  const textoT    = topD.textoT    || '';
  const textoGrafM = topD.textoGrafM || '';
  const textoGrafL = topD.textoGrafL || '';
  const textoGrafT = topD.textoGrafT || '';

  // Tres combinaciones (por factores ordenados)
  function _comb(f1, f2) {
    if (!f1 || !f2) return {};
    const k  = f1 + f2;
    const pf = PERFILES_DISC[k] || PERFILES_DISC[f1] || {};
  ...
  return { sc, neto, top, sec, key, perfil, fortalezas, areas, textoM, textoL, textoT, textoGrafM, textoGrafL, textoGrafT, combs };
}
...
function _yaRespondio(correo) {
  try {
    ...
    [CFG_APP.HOJA_PLANTILLA, CFG_APP.HOJA_CANDIDATOS].forEach(function(nombre) {
      if (encontrado) return;
      const h = ss.getSheetByName(nombre);
      if (!h || h.getLastRow() < 2) return;
      const datos = h.getRange(2, esCandidatoSheet(nombre) ? 4 : 2, h.getLastRow()-1, 1).getValues();
      if (datos.some(function(r){ return String(r[0]).trim().toLowerCase() === correo; })) encontrado = true;
    });
    return encontrado;
...
function inicializarHojasApp() {
  ...
  SpreadsheetApp.getUi().alert(
    "Hojas listas (" + defs.length + " verificadas).\n\n" +
    "Próximos pasos:\n" +
    "1. Agrega dominios en '" + CFG_APP.HOJA_DOMINIOS + "'\n" +
    "2. Sincroniza tokens desde Nuevos_Ingresos\n" +
...
function sincronizarTokensDesdeNuevosIngresos() {
  try {
    ...
    SpreadsheetApp.getUi().alert(nuevos + " token(s) nuevos sincronizados en " + CFG_APP.HOJA_TOKENS + ".");
  } catch(e) {
